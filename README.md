...
| `--scene <scene>` | Non-shipping startup override. Opens one registered scene key or direct `.bsn` asset path instead of the default splash flow. | `scripts\run.cmd -- --scene last-beacon/main_menu` |
| `--scene [<scene>, <scene>]` | Non-shipping startup stack override. Opens scenes in order from bottom to top. Spaces around commas are allowed; quote the value when using spaces. | `scripts\run.cmd -- --scene "[last-beacon/gameplay_level, scenes/main_menu.bsn]"` |
