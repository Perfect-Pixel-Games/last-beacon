...
## Phase 7: Prepared-instance preload caching (Part B)
**Status:** Planned — awaiting explicit user approval before implementation, per `feature-plan-docs`'s mandatory-stop rule
**Goal:** Opening an actually-preloaded scene is instant (zero visible construction); see `plan.md`'s "2026-08-23 Scope Expansion" section for the full design.

### Tasks
- [ ] Add `PreparedSceneCache` resource to `scene_stack.rs`.
  - Status: Planned
  - Repository: `engine`
- [ ] Extend `prepare_registered_scene_preloads` (renamed from `warm_registered_scene_preloads`) to spawn real off-stack content, not just warm the asset.
  - Status: Planned
  - Repository: `engine`
- [ ] Extend scene-open command processing (`Streaming` and `Blocking`) to check the cache first: instant activation on ready hit, reuse on in-progress hit, unchanged fallback on miss.
  - Status: Planned
  - Repository: `engine`
- [ ] Stale-entity safety net: verify cached entity still exists before activating; fall back to fresh spawn if not.
  - Status: Planned
  - Repository: `engine`
- [ ] Tests per `plan.md`'s Testing Methodology (cache hit/miss/in-progress, stale entity, no duplicate construction).
  - Status: Planned
  - Repository: `engine`

### Validation
- Engine validation: `Pending`
- Documentation generation: `Pending — engine/docs/scene-system.md`
- User confirmation: `Required before implementation starts (not yet given)`
...
- `2026-08-23` (follow-up): User confirmed the feature works and requested the remaining concrete preload registrations: `main_menu → options_menu` (background) and `hangar → dashboard/garage/mission_control/fabrication/silo_upgrades` (all background). Asked the user what "child hangar scenes" meant since no sub-scenes are registered under `HANGAR_SCENE` in the catalog — confirmed it means the other Beacon-shell pages. TDD: 3 new tests (`main_menu_preloads_options_menu`, `hangar_preloads_every_other_beacon_page`, `no_other_preload_relationships_are_registered` — the last explicitly enforces the user's "no more, no less" constraint by asserting every other scene key has zero preload targets). Confirmed red, implemented, confirmed green. Game validation clean (format, clippy, all tests). Manual smoke test clean. `ScenePreloadMode::Blocking` remains unused for all four relationships — the user only asked for background warming.
- `2026-08-23` (bug report — Part A): User reported a visible "fizzle" (fonts loading, padding adjusting, assets drawing) even on preloaded/streamed scenes, and required zero fizzle for every scene regardless of load mode. Root-caused: `apply_last_beacon_ui_font` called `asset_server.load(...)` fresh on every call with no persistent strong reference anywhere except the `TextFont` components themselves, which despawn with their scene — when a scene closes and its replacement's text doesn't exist yet (a real gap for same-frame Beacon-page `Close`+`Open` navigation), the font's ref count could hit zero, Bevy unloads it, and the next scene's text has to reload it from scratch (fallback glyphs → real font pop). Font metrics changing on that pop very plausibly explains the "padding adjusts" symptom too as one cascading effect, not two separate causes — deliberately implemented and tested the font fix alone first before considering a speculative layout-timing fix, per systematic-debugging discipline. Fix: `LastBeaconUiFontHandles` resource holds a permanent strong reference to both fonts (loaded once via `FromWorld`, degrading to default handles if `AssetServer` isn't present — mirrors `FoundationBsnAssetPlugin`'s own AssetServer-presence check — so a pre-existing minimal test keeps working); `apply_last_beacon_ui_font` now clones from that persistent resource instead of reloading, and marks newly-spawned text `SceneContentLoading` if its font isn't `Loaded` yet; new `reveal_last_beacon_text_once_fonts_load` system clears that marker once both fonts finish loading (a one-time event early in the session, invisible afterward). TDD: 4 new tests. Full game validation clean (20 lib + 2 integration tests, clippy, fmt). Manual smoke test clean. Root commit `1e2c453` pushed.
- `2026-08-23` (Part B planning): User confirmed the font fix should be paired with a real fix for "preloaded scenes aren't actually instant." Wrote up `PreparedSceneCache` design in `plan.md`'s "2026-08-23 Scope Expansion" section (Phase 7 below) — deliberately reuses the existing Phase 2/3 readiness and off-stack-spawn machinery instead of introducing new lifecycle state, specifically to avoid the failure modes that sank the original branch (no readiness-token API, no automatic refill, no new resolve-caching code, stale-entity fallback instead of cross-module hot-reload invalidation). Stopping here per `feature-plan-docs`' mandatory-stop rule and the user's own earlier request to plan this properly before touching it — awaiting explicit approval before implementation.
- `2026-08-23` (bug report — Part A follow-up, ordering gap): User reported the font pop was still visible after Part A, plus a new "hitch." Root-caused a second, distinct issue: `apply_last_beacon_ui_font` had no explicit ordering relative to the systems that actually create text (`apply_pending_bsn_instances` in the engine, `apply_pending_last_beacon_bsn_widgets` in the game, each registered in separate `add_systems` calls with no `.chain()`/`.after()` between them). Without that ordering, newly-created text could render for one full frame with its unauthored `.bsn`-default font before this system corrected it on the following frame — a real per-transition font pop, distinct from Part A's asset-unload issue. Made `apply_pending_bsn_instances` `pub` (engine) with a doc comment stating the ordering requirement; reordered `apply_last_beacon_ui_font`/`reveal_last_beacon_text_once_fonts_load` into their own chained system group `.after()` both content-creating systems. New regression test `scene_text_never_shows_the_wrong_font_even_transiently` (integration test, `bsn_asset_flow.rs`) checks the invariant on *every* frame across 600 frames, not just at the end, specifically because a one-frame lag would pass an eventually-correct assertion; confirmed it fails when the ordering is deliberately inverted (`.before()` instead of `.after()`) before restoring the fix, per TDD discipline. Full validation clean (23 game tests, clippy, fmt on both repos). Manual smoke test clean. Engine commit `6faaf44`; root commit pending. Did not yet re-investigate the reported "hitch" separately — asked the user to re-test both symptoms after this fix before further diagnosis, since the ordering bug may also explain it (a scene-wide batch of text corrections landing on the wrong frame could plausibly read as a stutter, not just a pop).
- `2026-08-23` (bug report — Settings not working from the pause menu): User confirmed the font-pop/hitch fix worked, then reported Settings doesn't work when opened from the pause menu. Root-caused (code-evidence chain, no live reproduction needed — deterministic, not timing-dependent): `game/src/lib.rs` registered `apply_pending_last_beacon_bsn_widgets` (the system that clears the `SceneContentLoading` marker Phase 2 put on nested BSN widget slots) with `.run_if(foundation_is_not_paused)`. Opening the pause menu sets `FoundationPauseState.paused = true` (`menu.rs`) and it stays `true` for as long as anything is layered on top, including the options menu opened from it. `options_menu.bsn` contains a nested `LastBeaconBsnWidget` (the divider), so its widget-slot loading marker could never clear while paused — with Phase 2's readiness gating, the entire scene stayed hidden forever. `pause_menu.bsn` itself has no nested widget, which is why only the settings-from-pause path broke (main-menu settings and the pause menu itself were unaffected). This was a latent bug in a pre-existing pause gate, only surfaced into a full "never becomes visible" failure by this feature's own Phase 2 readiness gating — not present before this feature (pre-Phase-2, the scene would just show with a missing divider). Fix: removed the `run_if(foundation_is_not_paused)` gate — it's a generic content-loading system, not gameplay simulation, and must run regardless of pause state. TDD: new regression test `widget_bearing_scene_still_reveals_while_gameplay_is_paused` (`bsn_asset_flow.rs`) mirrors the production registration inline (matching the existing font-pop test's convention), sets `FoundationPauseState.paused = true`, loads `options_menu.bsn`, and asserts no entity is left with `SceneContentLoading` after 600 frames; confirmed red (`left: 1, right: 0`) with the gate present, green after removing it. Full game validation clean (24 tests: 20 lib + 4 integration), clippy `-D warnings` clean. Root commit `e04770f` pushed.
- `2026-08-24` (branch consolidation — BSN parser panic safety): Following a full codebase review (see [[project_last_beacon_overview]] context), planned and implemented a fix for four `.unwrap()` panics in the engine's `dynamic_bsn.rs` `.bsn` parser (`DynamicBsnLoaderError::UnknownType` guard, matching the identical guard already used by structurally equivalent code paths in the same file) on a dedicated `feature/bsn-parser-panic-safety` branch created from `dev` in both repos, with its own `docs/plans/bsn-parser-panic-safety/` plan and tracker. After implementation, the user noticed the font pop-in fix appeared to have regressed while testing — root-caused to testing on that separate `dev`-based branch, which legitimately lacked every async-scene-loading fix (not an actual regression). The user asked to consolidate onto this branch instead, so only one branch is active at a time. Cherry-picked the engine fix (`3b32ed8` → `004f7f1`, clean, no conflicts since it touches an unrelated file) and the plan/tracker doc commit (`99d7d87` → `fb9d2cc`) onto `feature/async-scene-loading` in both repositories, updated the root `engine` submodule pointer to `004f7f1`, and re-verified full validation on the consolidated branch (114 engine tests, clean clippy/fmt/doc; game tests and clippy clean). The original `feature/bsn-parser-panic-safety` branches remain pushed to `origin` in both repos but are now superseded; left in place pending the user's decision on cleanup.
