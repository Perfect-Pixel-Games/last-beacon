...
- Engine submodule pointer: `Updated to prepared scene propagation performance engine commit b4e7318bdc90fa68a436f8b8a2532253e8297081; root pointer committed in 739c6ba and pushed`
- Overall status: `Phase 7 performance follow-up validated, committed, and pushed`
- Planning model: `gpt-5.5`
- Preferred implementation model: `gpt-5.4`
- Optional final review model: `gpt-5.5`
- Current handoff state: `Implementation validated with gpt-5.4 — optimized preparation-context propagation and tunable BSN apply budget`
- Created: `2026-07-20`
...
- `2026-07-22`: User reported a startup regression after main-menu root orchestration: splash screens and main menu no longer appeared, and after several seconds the game showed the green rotating placeholder cube. Investigated cached BSN activation and found prepared BSN descendants could miss their active `SceneOwner` when activation depended on the lower-level `SceneLoadRequested` path and recursive ownership propagation used potentially stale `Children` relationships. Changed cached activation to reconcile prepared roots against the authoritative `SceneStack`, propagate `SceneOwner` through direct `ChildOf` links, guard activation so standalone BSN loading still works without `FoundationSceneStackPlugin`, and updated `game/tests/bsn_asset_flow.rs` to exercise the public `SceneCommand::open` path with an ownership assertion on authored splash text. Focused regression tests passed, followed by full validation: `engine/scripts/validate-project.cmd` and `scripts/validate.cmd` both exited 0. Engine fix committed as `88fd955fd1e068ae4d75bc798a33e55a3df3908f`; root submodule pointer/test/tracker committed as `30f7648c2f2aa2939382f15d60636877f5126414`; engine and root commits pushed to their `origin/feature/scene-pop-in-investigation` branches.
- `2026-07-22`: User reported that splash screens and the main menu only flashed, the game became very unperformant for roughly 10 seconds, and a second green cube appeared. Root cause: the cached activation system reconciled every stack entry every frame, so after a scene consumed a prepared cache copy and requested a background refill, the refilled copy was activated into the already-populated scene again, repeatedly duplicating content and triggering repeated BSN apply work. A secondary issue allowed Last Beacon placeholder cube markers inside off-stack prepared cache copies to spawn unowned cube/camera/light entities before activation. Fixed Foundation cached activation to skip scenes that already have an active `FoundationBsnInstance` for their `SceneOwner`; fixed Last Beacon placeholder generation to require an effective `SceneOwner`; added regression tests for both. Validation passed: `engine/scripts/validate-project.cmd` and `scripts/validate.cmd` both exited 0. Engine fix committed as `fc002b87cd985d0365b6357e1b9bfc1554ecaa9e`; root submodule pointer/test/tracker committed as `7a9ac2c1b2173fca766333dd1f96cb552e64949c`; engine and root commits pushed to their `origin/feature/scene-pop-in-investigation` branches.
- `2026-07-22`: User reported Pixel Perfect now appears consistently, but Bevy splash appears inconsistently and the flow never reaches the main menu. Root cause: `FoundationSplashRuntimeSettings` defaults to allowing unowned splash components, while prepared/cache copies are intentionally unowned until activation. Last Beacon preloads both splash BSNs, so those off-stack prepared copies could initialize and advance their `FoundationSplashRuntime` before activation; Bevy could then activate already partially/fully completed, causing inconsistent display and a stuck root sequencer waiting for an active-scene completion. Fixed Last Beacon to require scene ownership for splash runtime processing and added `last_beacon_splash_runtime_requires_scene_ownership`. Validation passed: `scripts/validate.cmd` exited 0. Root commit `ed1b1b0` pushed.
- `2026-07-22`: User reported remaining bugs: no green cube in main menu, Pixel Perfect does not load in time for fade-in, and pressing Escape to skip the current splash results in a black screen. Root cause: the main-menu root started Pixel Perfect immediately while Bevy/main-menu/options preloads were still preparing. When a splash naturally completed or was skipped before the next target was ready, the completed splash stayed transparent over the black root while the queued transition waited for preparation. Fixed `advance_last_beacon_main_menu_roots` to request and wait for the full Main Menu preload group before opening Pixel Perfect, so Pixel/Bevy/main_menu/options are cached before any splash timer can start. Added tests for waiting before preloads and starting once all are ready. Validation passed: `scripts/validate.cmd` exited 0. Root commit `ea6bb09` pushed.
- `2026-07-22`: User reported the issues still persisted. Strengthened the Foundation-side invariant: scene-owned splash timers and Escape skip handling now advance only when the owning scene stack entry is actually visible. This prevents fade/skip/completion from running while a scene is still hidden by readiness/activation, preserving standalone unowned behavior. Added `scene_owned_splash_advances_only_when_stack_visible`. Validation passed: `engine/scripts/validate-project.cmd` and `scripts/validate.cmd` both exited 0. Engine fix committed as `88f89d8fab415dfdb21fc36d78e7112dfb196903`; root submodule pointer/tracker committed as `08269c9` and pushed.
- `2026-07-22`: User reported Escape now closes any scene and clarified Escape should only skip the current cutscene, close Settings, or close Pause depending on which is top of stack. Fixed Foundation Escape handling to require `FoundationCloseOnEscape` and splash skip targets to be on the focused stack entry, preserving standalone behavior; added a regression test proving a close marker on a lower scene does not close the focused scene. Removed `FoundationCloseOnEscape` from Last Beacon credits so credits no longer participates in the allowed Escape-close set. Validation passed: `engine/scripts/validate-project.cmd` and `scripts/validate.cmd` both exited 0. Engine fix committed as `55a2a8194b4a86b88c5963429004ebe20cee40a3`; root submodule pointer/credits/tracker committed as `41d1193` and pushed.
- `2026-07-22`: User reported Escape still closed more scenes than intended, e.g. Beacon, and suspected splash scenes were also being closed. Root cause: unowned `FoundationCloseOnEscape`/splash components from prepared off-stack cache copies could still act while a scene stack existed because only focused owned handlers were filtered; unowned handlers were treated as standalone. Fixed Foundation to ignore unowned close/skip handlers whenever a `SceneStack` exists, preserving no-stack standalone behavior. Added regression test proving an unowned `FoundationCloseOnEscape` marker cannot close a focused Beacon scene. Validation passed: `engine/scripts/validate-project.cmd` and `scripts/validate.cmd` both exited 0. Engine fix committed as `b6c95b7001c1348b870d95f82b24cd5c20030d75`; root submodule pointer/tracker committed as `6abb0d7` and pushed.
- `2026-07-22`: User confirmed Escape behavior worked, but the main menu had no green cube / later noted the cube loaded later than the menu and should be part of preload. Root cause: `LastBeaconPlaceholderCubeScene` was authored in cached `main_menu.bsn` while unowned; earlier fixes skipped unowned markers to prevent leaked cubes, then retried only after activation, delaying cube/camera/light generation until after the menu appeared. Fixed Last Beacon placeholder handling to generate cube/light/inactive camera during preload as children of the authored marker, keep them unowned and inactive while off-stack, then assign `SceneOwner`, activate the camera, and mark initialized once the scene activates. Added regression coverage for inactive prepared generation and later scene-owner activation. Validation passed: `scripts/validate.cmd` exited 0. Root commit `1a89117` pushed.
- `2026-07-22`: User reported the cube no longer loads at all but must be ready by main-menu open via preload. Root cause: the preloaded generated 3D cube/camera/light were parented under the authored main-menu UI root, causing them to inherit UI hierarchy state/transforms instead of behaving like independent 3D scene content. Fixed generated placeholder entities to stay top-level while still being generated during preload, with camera inactive until scene activation and ownership assignment. Added regression coverage that preloaded 3D placeholder content has no `ChildOf` relationship. Validation passed: focused placeholder tests and `scripts/validate.cmd` exited 0. Root commit `4300378` pushed.
- `2026-07-22`: User reported two green cubes and remaining render delay, and requested a full check rather than a hacky fix. Full root-cause pass found the top-level generated placeholder entities could still leak from prepared/refilled cache copies, and activation still required a later game update because generated entities were not part of the prepared BSN root ownership/cleanup lifecycle. First attempted to parent generated 3D content to the nearest `FoundationBsnInstance` root, but user reported the cube still did not appear; follow-up analysis identified this could still inherit non-3D BSN/UI hierarchy behavior. Corrected the lifecycle again: generated cube/light/inactive camera are created during preload as hidden top-level 3D entities, remain hidden/inactive while off-stack so prepared/refill copies cannot render duplicates, then receive `SceneOwner`, `Visibility::Inherited`, and an active camera in Last Beacon's PostUpdate pass after Foundation scene activation but before scene visibility sync. Added regression coverage for hidden top-level prepared content and active reveal on ownership assignment. Validation passed: focused placeholder tests and `scripts/validate.cmd` exited 0. Root commit `22fb5aa` pushed.
...
- `2026-07-22`: User reported a Beacon load `exit code: 1` and worse performance. Direct reproduction with `cargo run --manifest-path game/Cargo.toml -- --log-inline --scene last-beacon/beacon` did not crash; the game opened and continued running until the local timeout killed it, so the exit-code path still needs a user log or a more exact reproduction path. Found a concrete performance regression in the Phase 7 implementation: prepared-cache context propagation scanned all `ChildOf` links recursively per entity (O(n²) on large BSN trees), and the one-apply-per-frame default stretched large preload groups too aggressively. Fixed propagation to build the child map once and walk the subtree linearly. Made `FoundationBsnPreparationBudget` default to 2 applies per frame and tunable through `FOUNDATION_BSN_APPLY_BUDGET`. Validation passed: focused BSN tests, `engine/scripts/validate-project.cmd`, and `scripts/validate.cmd` exited 0. Engine commit `b4e7318bdc90fa68a436f8b8a2532253e8297081` pushed; root pointer commit `739c6ba` pushed.
