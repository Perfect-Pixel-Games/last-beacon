...
- Overall status: `BSN-root preloaded placeholder lifecycle fix validated; commit/push pending`
...
## Repository State
- Root commit/push state: `Top-level placeholder correction commits through 179f12b pushed to origin/feature/scene-pop-in-investigation; BSN-root preloaded placeholder lifecycle fix pending commit`
...
- `2026-07-22`: User reported two green cubes and remaining render delay, and requested a full check rather than a hacky fix. Full root-cause pass found the top-level generated placeholder entities could still leak from prepared/refilled cache copies, and activation still required a later game update because generated entities were not part of the prepared BSN root ownership/cleanup lifecycle. Corrected the lifecycle: generated cube/light/inactive camera are created during preload, parented to the nearest `FoundationBsnInstance` root (not the UI node), inherited into Foundation's activation/cleanup ownership, and activated in Last Beacon's PostUpdate pass after Foundation scene activation but before scene visibility sync when scene ownership is present. Added regression coverage for the real cached-BSN shape: generated cube is parented under the BSN root. Validation passed: focused placeholder tests and `scripts/validate.cmd` exited 0. Root commit pending.
