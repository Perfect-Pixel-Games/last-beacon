...
## Scope Expansion: Logging Visual Polish
...
- Follow-up clarification: `--log` should open a separate Foundation log window by default; standalone `--log-inline` should opt into reusing the current parent terminal without also requiring `--log`.
- Separate Windows log consoles should enable ANSI/virtual-terminal styling and use conservative aligned text so colors and columns render consistently.
