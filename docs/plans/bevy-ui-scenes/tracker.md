...
- Current tweak state: `Number field now uses [-][value][+] with the center value as the only numeric editable input, no gaps between boxes, preserved authored layout, numeric input filtering, and stable value sync; slider now has no internal label and shows percent in a centered right-hand slot; validation passed; checkpoint commit pending.`
